...
<template>
  <button
    ...
    :class="{
      ...
      'border border-primary dark:border-sky-700': !props.isConnected,
      'border border-gray-600': pending,
    }"
  >
    <IconCheckDecagram class="w-[28px] fill-light" v-if="props.isConnected" />
    <IconPlus class="w-[20px] sm:w-[26px] fill-primary dark:fill-sky-700" v-if="!props.isConnected && !pending" />
    <IconClockOutline class="w-[20px] sm:w-[26px] fill-primary dark:fill-sky-700" v-if="!props.isConnected && pending" />
